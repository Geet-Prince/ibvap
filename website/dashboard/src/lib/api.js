...
export function normalizeAlert(raw) {
  ...
  return {
    ...
    bbox: ubbox,
    snapshotUrl: snapshotUrl(incidentId, snapshot),
...
export function normalizeIncident(raw) {
  ...
  return {
    ...
    snapshotUrl: snapshotUrl(raw.incident_id, raw.snapshots?.[0]),
    ...
    snapshots: (raw.snapshots || []).map((f) => snapshotUrl(raw.incident_id, f)),
    zoneBreaches: raw.zone_breaches || [],
