...
export default function DashboardLayout() {
  ...
  // up on anything missed. Runs on mount AND whenever the socket reconnects —
  ...
  // Track socket status → feed health.
  useEffect(() => {
  ...
  // Derive known cameras from real data + the guaranteed live source.
  const cameras = useMemo(() => {
    const map = new Map();
    map.set('CAM_LIVE', { id: 'CAM_LIVE', name: 'Cam Live' });
    [...alerts, ...incidents].forEach((a) => {
      if (a.cameraId && !map.has(a.cameraId)) map.set(a.cameraId, { id: a.cameraId, name: a.cameraName });
    });
    return Array.from(map.values());
  }, [alerts, incidents]);

  const activeCamera = cameras.find((c) => c.id === 'CAM_LIVE') || cameras[0];

  // Detection boxes for the feed overlay — recent alerts that carry a bbox.
  ...
  // Human count shown on the feed HUD — reads the pipeline's per-frame live
  // count (same source as the feed), NOT the historical alert aggregate that
  // backs the "Humans" stat card. null → live source unknown, show "--".
        ...
        <div className="grid grid-cols-1 gap-4 min-[900px]:grid-cols-[340px_minmax(0,1fr)_340px] min-[900px]:items-start">
          <div className="min-[900px]:order-1 max-[899px]:order-3">
            <CameraPanel
              ...
              onSelectCamera={() => setSelectedId(null)}
